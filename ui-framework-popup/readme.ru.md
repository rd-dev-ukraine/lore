...
В версии 1.5 Angular приобрел синтаксический сахар в виде метода `component` у модуля, 
который позволяет создавать компоненты. 
Компоненты это на самом деле директивы, но код их объявления ориентирован на создание 
кирпичиков предметной области приложения,
...
т.к. разметка получается маленькая и аккуратная - так как используются вложенные компоненты, 
...
Я покажу два из них, один при помощи `$compile`, второй с использованием директивы с `transclude`.
...
Также этот способ применяется, если тип компонента неизвестен.
...
Сервис `$compile` позволяет скомпилировать строку с Angular разметкой в DOM элемент и связать его со `$scope`.
...
Здесь нужная директива - `<popup>...</popup>`. 
Все, что внутри нее, будет показано во всплывающем окне.

Небольшой недостаток этого метода в том, что показывать и прятать окно необходимо при помощи `ng-if`,
...
Напрямую использовать контент (например, через `$element.children`) нельзя - он не связан с правильным скоупом, 
...
Она, однако, не возвращает значение, как обычная фабрика, а передает его в коллбек-функцию.

Можно создавать сколько угодно копий, переопределять им `scope`, добавлять в любое место документа, и так далее.
Круто.
...
Очищать контент нужно в `$onDestroy`.

Осталось последнее - как получить доступ к transclude function. 
Она передается на самом деле в нескольких местах, но мы ее заинжектим в контроллер.
Для того, чтобы она передалась, в директиве должно быть указано `transclude: true`.
...
* Привязано к текущему scope.

Недостатки:
* В этом варианте нужно использовать `ng-if` для управления показом.
